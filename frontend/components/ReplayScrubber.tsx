...
export function ReplayScrubber({
  ...
  return (
    <div className="bg-[#161B22] border border-[#C5A059]/40 rounded-xl p-5 shadow-lg glow-brass">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
              isPlaying
                ? "bg-[#F0A202] text-black hover:bg-[#F0A202]/90 shadow-md"
                : "bg-[#C5A059] text-black hover:bg-[#C5A059]/90 shadow-md"
            }`}
          >
          ...
          <button
            onClick={onReset}
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-[#0D1117] hover:bg-white/10 text-[#8B949E] hover:text-white border border-white/10 transition-all"
          >
            ...
            {[0.5, 1, 2, 4, 8].map((s) => (
              <button
                ...
                className={`px-2 py-0.5 rounded text-[11px] font-medium transition-all ${
                  speed === s ? "bg-[#21262D] text-[#C5A059] font-bold" : "text-[#8B949E] hover:text-white"
                }`}
              >
                {s}x
              </button>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <div className="text-lg font-mono font-bold text-[#C5A059] flex items-center gap-1.5 justify-end">
              <Clock className="h-4 w-4" />
              <span>{getSimulatedDate()}</span>
            </div>
          </div>
          <div className="px-2.5 py-1 rounded bg-white/[0.04] border border-white/10 text-xs font-mono text-white">
            {progressPercent}%
          </div>
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        <input
          ...
          className="w-full h-2 bg-[#0D1117] rounded-lg appearance-none cursor-pointer accent-[#C5A059]"
        />
